...
const loginPrompt = async context => {
  ...
  const sockets = new Sockets('AuthenticationChannel', apiUrl, proxyUrl);
  try {
  ...
  return new Promise(resolve => {
    sockets.emit('authorization_init').on('authorization_init', data => {
      const url = new URL('/account/cli', apiUrl);
      // Adds SID(SocketSessionID) and NAME params required for authentication
      url.searchParams.append('name', name);
      url.searchParams.append('sid', data.uuid);
      // Adds login action required for dispaying a correct message on the UI of LD
      if (loginAction) {
