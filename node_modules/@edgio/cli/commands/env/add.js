...
exports.handler = async yargs => {
  ...
  const propertySlug = validator.validatePropertyName(localPropertyParam || config.name || packageJson.name);
  const organizationSlug = localOrganizationParam || config.organization || config.team;
  ...
  const environmentDetails = [`> organization=${organizationSlug || 'Private space'}`, `> property=${propertySlug}`, `> environment=${environmentParam}`, `> production=${productionParam}`, `> canMembersDeploy=${canMembersDeployParam}`].filter(Boolean); // Removes falsy values
