...
// We need to set version from package.json
...
const validateParameters = function (parameters) {
  ...
  parameters.img = url.format(u);

  // stripWWW: false because we need to keep the original host name.
  ...
  parameters.fmt = constants.formatJpeg;

  // If the "Accept" header exists, use that. Prefer image fmts in this order: webp, png, jgp
...
const optimizeImage = async function (parameters) {
  ...
  parameters.imageBuffer = sharp(parameters.buffer);

  // If format is Jpeg and there is a transparency layer in the original, use webp or png because jpeg does not support transparency.
...
const buildResponseHeaders = function (parameters) {
  ...
  const x0image = parameters.isOldXdnRequest ? 'x-xdn-image' : 'x-0-image';

  // These base headers are returned on both success and failure
  ...
  responseHeaders[x0image] = `i=${parameters.img}` + `${parameters.fmt ? ',f=' + parameters.fmt : ''}` + `${parameters.height ? ',h=' + parameters.height : ''}` + `${parameters.width ? ',w=' + parameters.width : ''}` + `${parameters.quality ? ',q=' + parameters.quality : ''}` + `${parameters.iSize ? ',i=' + parameters.iSize : ''}` + `${parameters.oSize ? ',o=' + parameters.oSize : ''}`;

  // If we went upstream, include that status code in the reponse headers
