...
const setProxy = require('./prompts/setProxy');
...
yargs.scriptName('edg').middleware(initContext).commandDir('commands').command({
  command: 'curl',
  describe: 'edgio curl client. Refer to "edg curl --help" for documentation'
}).option('api-url', {
  type: 'string',
  ...
  type: 'string',
  description: "HTTP/S proxy server URL to use for all requests made by @edgio/cli. You can also specify the proxy URL globally by setting the 'CLI_PROXY_URL' environment variable.\r\nExample: CLI_PROXY_URL=https://proxy.local:8443"
}).middleware(setProxy).options('token', {
  type: 'string',
  description: "Authenticate with a specific property's deploy token. You can also specify the token by setting the EDGIO_DEPLOY_TOKEN environment variable."
}).option('verbose', {
  alias: 'v',
  type: 'boolean',
